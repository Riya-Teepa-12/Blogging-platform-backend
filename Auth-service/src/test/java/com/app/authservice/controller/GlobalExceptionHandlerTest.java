...
class GlobalExceptionHandlerTest {
    ...
    void handleIllegalStateReturnsConflict() {
        ...
        Map<String, String> body = handler.handleIllegalState(new IllegalStateException("conflict")).getBody();

        assertThat(handler.handleIllegalState(new IllegalStateException("conflict")).getStatusCode())
                .isEqualTo(HttpStatus.CONFLICT);
        assertThat(body).containsEntry("message", "conflict");
    }

    @Test
    void handleValidationReturnsFieldMessage() {
        GlobalExceptionHandler handler = new GlobalExceptionHandler();
        MethodArgumentNotValidException ex = createValidationException("email", "must not be blank");

        Map<String, String> body = handler.handleValidation(ex).getBody();

        assertThat(body).containsEntry("message", "email must not be blank");
    }
