package com.app.mediaservice.config;
...
class HttpRequestLoggingFilterTest {
    ...
    void shouldSkipFrameworkPaths() {
        ...
        assertThat(filter.shouldNotFilter(new MockHttpServletRequest("GET", "/media"))).isFalse();
    }

    @Test
    void doFilterInternalRunsChain() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/media");
        MockHttpServletResponse response = new MockHttpServletResponse();
        FilterChain chain = (req, res) -> ((MockHttpServletResponse) res).setStatus(206);
